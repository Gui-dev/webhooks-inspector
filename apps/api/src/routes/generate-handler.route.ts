...
import { GenerateHandlerUseCase } from '@/use-cases/generate-handler.use-case'
  ...
  app.post(
    '/generate',
    {
      schema: {
        ...
        tags: ['webhooks'],
        body: z.object({
          webhooksIds: z.array(z.string()).min(1),
        }),
        response: {
          201: z.object({
            code: z.string(),
    ...
    async (request, reply) => {
      ...
      const generateHandlerUseCase = new GenerateHandlerUseCase(webhookRepository)
      const { code } = await generateHandlerUseCase.execute({ ids: webhooksIds })
      ...
      reply.status(201).send({ code: text })
    }
  )
}
